...
def _test_table_has_11_rows():
    _ck(len(CHECKS) == 11)
    _ck(len(set(row[0] for row in CHECKS)) == 11)  # every id unique
...
def _test_forbidden_location_refuses_unc_share_crossing_and_drive_relative_paths():
    ...
    for event in cases:
        ...
        _ck(fired is not None, "an ambiguous Windows pathname must not be admitted")
        _ck(fired[0] == "ward.forbidden_location")
        _ck("outside the working directory" in fired[1])


def _test_forbidden_location_does_not_recommend_inapplicable_line_exemption():
    fired = evaluate(_write("x = 1\n", file_path="/etc/ward.conf"))
    _ck(fired is not None)
    _ck("ward-allow" not in fired[1])
...
def _test_timing_unsafe_compare_fires():
    ...
    _ck(fired is not None and fired[0] == "ward.timing_unsafe_compare")


def _test_timing_unsafe_compare_clean_with_compare_digest():
    _ck(evaluate(_write("if hmac.compare_digest(computed.hexdigest(), provided):\n    pass\n")) is None)
...
def _test_timing_unsafe_compare_clean_on_polysemous_word():
    ...
    _ck(evaluate(_write("if config_key == expected_key:\n    pass\n")) is None)
...
def _test_timing_unsafe_compare_clean_on_direct_sentinel_comparison():
    _ck(evaluate(_write("if computed.hexdigest() != None:\n    pass\n")) is None)
...
def _test_jwt_signature_disabled_fires_on_verify_false():
    ...
    _ck(fired is not None and fired[0] == "ward.jwt_signature_disabled")
...
def _test_cert_verify_disabled_fires_on_requests_verify_false():
    ...
    _ck(fired is not None and fired[0] == "ward.cert_verify_disabled")
...
def _test_cert_verify_disabled_clean_on_unrelated_verify_kwarg():
    ...
    _ck(evaluate(_write("form.clean(verify=False)\n")) is None)


def _test_cert_verify_disabled_clean_when_library_name_is_only_a_substring():
    _ck(evaluate(_write("form.myrequests(verify=False)\n")) is None)
...
def _test_cert_verify_disabled_clean_when_verified():
    _ck(evaluate(_write("requests.get(url, verify=True)\n")) is None)
...
def _test_cert_reqs_none_clean_when_required():
    _ck(evaluate(_write("ssl.wrap_socket(sock, cert_reqs=ssl.CERT_REQUIRED)\n")) is None)


def _test_cert_reqs_none_clean_on_unrelated_function():
    _ck(evaluate(_write("render(cert_reqs=CERT_NONE)\n")) is None)
...
def _test_paramiko_host_key_weakened_fires_on_autoadd():
    ...
    _ck(fired is not None and fired[0] == "ward.paramiko_host_key_weakened")
...
def _test_paramiko_host_key_weakened_names_warning_policy_correctly():
    ...
    _ck(fired is not None and "set_missing_host_key_policy(WarningPolicy)" in fired[1])


def _test_paramiko_host_key_weakened_clean_on_reject_policy():
    _ck(evaluate(_write("client.set_missing_host_key_policy(paramiko.RejectPolicy())\n")) is None)
...
def _test_outbound_secret_pattern_fires_on_webfetch():
    ...
    _ck(fired is not None and fired[0] == "ward.outbound_secret_pattern")
...
def _test_self_mute_guard_catches_planted_verifier_removal():
    ...
    fired = evaluate(event)
    _ck(fired is not None and fired[0] == "ward.self_mute_guard")
...
def _test_integrity_suppression_flag_catches_planted_env_gate():
    ...
    fired = evaluate(event)
    _ck(fired is not None and fired[0] == "ward.integrity_suppression_flag")
...
def _test_integrity_suppression_flag_negative_plant_depends_on_introduced_input():
    ...
    _ck(evaluate(event) is not None, "planted suppression must exercise the check")
    event["tool_input"]["content"] = "integrity_verification_enabled = true\n"
    _ck(evaluate(event) is None, "removing the planted suppression must remove the denial")
...
def _test_ward_allow_suppresses_ast_scaffolded_check():
    ...
    _ck(evaluate(_write(content)) is None)
...
def _test_write_missing_required_inspection_input_fails_closed():
    ...
    _ck(evaluate(complete) is not None)  # control: the complete unsafe action is denied
    ...
    for event in (missing_path, missing_input):
        fired = evaluate(event)
        _ck(fired is not None, "a Write Ward cannot inspect must not become an allow")
        _ck(fired[0] == "ward.cannot_evaluate")
        _ck("cannot evaluate" in fired[1])
...
def _test_empty_required_mutation_input_fails_closed():
    ...
    for event in cases:
        fired = evaluate(event)
        _ck(fired is not None, "an empty mutation must not become a quiet allow")
        _ck(fired[0] == "ward.cannot_evaluate")
        _ck("cannot evaluate" in fired[1])
...
def _test_edit_new_string_is_scanned_not_old_string():
    ...
    _ck(fragment is not None, "a detached security keyword must not become a clean result")
    _ck(fragment[0] == "ward.cannot_evaluate")
    _ck("fragment" in fragment[1])
...
def _test_multiedit_unparseable_fragments_fail_closed_without_joining():
    ...
    _ck(fired is not None)
    _ck(fired[0] == "ward.cannot_evaluate")
    _ck("cannot be parsed" in fired[1])
...
def _test_marker_in_an_unrelated_string_does_not_exempt():
    ...
    _ck(evaluate(_write(decoy + _UNSAFE)) is not None, "gate measured nothing -- the check lost its teeth")
...
def _test_marker_on_the_firing_line_still_exempts():
    ...
    _ck(evaluate(_write(annotated)) is None)


def _test_the_unsafe_control_fires_without_any_marker():
    _ck(evaluate(_write(_UNSAFE)) is not None)
...
def _test_one_annotated_line_does_not_carry_a_second_unsafe_line():
    ...
    fired = evaluate(_write(content))
    _ck(fired is not None, "gate measured nothing -- the check lost its teeth")
    _ck("line 3" in fired[1], fired)
...
def _test_a_marker_inside_a_docstring_does_not_exempt():
    ...
    _ck(evaluate(_write(content)) is not None)
